function hienDSThuocPX() {
  ...
  $.ajax({
    url: `https://backend-flame-nine.vercel.app/qlnt/phieuxuat/${cn}`,
    method: "GET",
    dataType: "json",
    success: function (data) {
      ...
      for (var i = 0; i < rows.length; i++) {
        rows[i].addEventListener("click", function () {
          ...
          console.log(cn);
          fetch(`https://backend-flame-nine.vercel.app/qlnt/phieuxuat/ct/${cn}/${sophieu}`)
...
function myFunctionpx(name) {
  ...
  input.addEventListener("change", function () {
    ...
    fetch(`https://backend-flame-nine.vercel.app/qlnt/kho/${cn}`)
      .then((response) => response.json())
      .then((data) => {
        var found = false;
        let tenBietDuoc = "";
...
function sukiensoLo(name) {
  ...
  fetch(`https://backend-flame-nine.vercel.app/qlnt/kho/sl/${maskusl}`)
    .then((response) => response.json())
    .then((data) => {
      console.log(data);
      let options = "";
...
function addPhieuXuat() {
  ...
  if (rows.length == 0) {
    ...
    // //
    if (flag == true) {
      fetch("https://backend-flame-nine.vercel.app/qlnt/phieuxuat/addPX", {
        method: "POST",
        headers: {
